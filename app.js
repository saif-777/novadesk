...
function initHub() {
  ...
  (function anim(t) {
    ...
    pickables.forEach((m) => {
      const raw = Math.min(1, Math.max(0, (time - m.userData.born) * 1.4));
      const grow = raw <= 0 ? 0.001 : easeOutBack(raw);
      m.scale.setScalar(grow * (m === hovered ? 1.12 : 1));
      m.position.y = m.userData.baseY + Math.sin(time * 1.2 + m.userData.phase) * 0.18;
      m.userData.ring.position.y = m.position.y;
      m.userData.ring.rotation.z += 0.003;
      m.userData.ring.material.opacity = 0.5 * Math.min(1, raw * 2);
...
function enterStation(s) {
  activeStation = s;
  spinFx = 1;
  camGoal = new THREE.Vector3(s ? meshX(s) : 0, 0.8, 5.2);
  lookGoal = new THREE.Vector3(meshX(s), 0.4, 0);
